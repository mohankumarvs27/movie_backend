...
const tmdbApi = {
  ...
  mediaVideos: async ({ mediaType, mediaId }) =>
    await axios.client.get(tmdbEndpoints.mediaVideos({ mediaType, mediaId })),
  mediaImages: async ({ mediaType, mediaId }) =>
    await axios.client.get(tmdbEndpoints.mediaImages({ mediaType, mediaId })),
  mediaRecommend: async ({ mediaType, mediaId }) =>
    await axios.client.get(
      tmdbEndpoints.mediaRecommend({ mediaType, mediaId })
    ),
  mediaSearch: async ({ mediaType, query, page }) =>
    await axios.client.get(
      tmdbEndpoints.mediaRecommend({ mediaType, query, page })
    ),
  personDetail: async ({ personId }) =>
    await axios.client.get(tmdbEndpoints.personDetail({ personId })),
  personMedias: async ({ personId }) =>
    await axios.client.get(tmdbEndpoints.personMedias({ personId })),
};
